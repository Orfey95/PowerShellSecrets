...
[string]$PasswordsHistory # Hidden field
}
# Creation of dictionary of secrets
$Global:DictionaryOfSecrets = @()
# Set path to secret file and key
...
function Get-SecretByID {
       ...
       if(!($Global:DictionaryOfSecrets | Where-Object {$_.ID -eq $IdOfSecret})) {
            Add-Secret "`nThere is no secret with such ID!"
       }
}
...
function Get-CryptographyKey {
       # Create Key 
       $Global:key = New-CryptographyKey -Algorithm AES -AsPlainText
       $Global:key | Out-File $Global:PathToKey
       echo "`n You have successfully created AES key!"
}
